...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Agouti address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Agouti: URI
...
{
    // Convert agouti:// to agouti:
    //
    //    Cannot handle this later, because agouti:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Agouti.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Agouti.lnk
...
{
    return GetAutostartDir() / "agouti.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a agouti.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Agouti\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the agouti app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add agouti app to startup item list
